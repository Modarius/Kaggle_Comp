...
def main():
   ...
   'relationship', 'race', 'sex', 'capital_gain', 'capital_loss', 'hours_per_week', 'native_country']
    ...
    for iter in np.arange(start=0, stop=50, step=1):
        ...
        aB, eh = ml.adaBoost(eh, attribs=attribs, T=1, prev_ensemble=aB)
        pred = pd.DataFrame(aB.HFinal(test_data),index=test_data.index, columns=['Prediction'])
        train_error[iter] = sum(training_data['label'] != aB.HFinal(training_data)) / len(training_data) # test the error for the given dataset
        pred.to_csv("labeled_data_aB_tree_" + str(iter+1) + ".csv",index=True, index_label='ID')
    print(train_error)
    return
